...
module Make_internal (Spec : Internal.CmdSpec [@alert "-internal"]) : sig
  ...
  val lin_test : pool:Util.Domain_pair.t -> count:int -> name:string -> QCheck.Test.t

  val neg_lin_test : pool:Util.Domain_pair.t -> count:int -> name:string -> QCheck.Test.t

  val stress_test : pool:Util.Domain_pair.t -> count:int -> name:string -> QCheck.Test.t
end
