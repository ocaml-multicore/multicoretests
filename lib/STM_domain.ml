...
module Make (Spec: Spec) = struct
  ...
  let check_quad_obs = check_quad_obs
  let all_interleavings_ok t = match t with
    | Spawn2 (seq_pref,cmds1,cmds2) -> all_interleavings_ok seq_pref cmds1 cmds2 Spec.init_state
    | Spawn3 (seq_pref,cmds1,cmds2,cmds3) -> all_quad_interleavings_ok seq_pref cmds1 cmds2 cmds3 Spec.init_state
  ...
  let arb_triple_asym seq_len par_len arb0 arb1 arb2 =
    ...
    let print_cmds = Print.list Spec.show_cmd in
    set_print (function (Spawn2 (seq,cs1,cs2)) -> print_triple_vertical ~center_prefix:false Spec.show_cmd (seq,cs1,cs2)
                      | (Spawn3 (seq,cs1,cs2,cs3)) ->  Print.quad print_cmds print_cmds print_cmds print_cmds (seq,cs1,cs2,cs3)) arb_triple
  ...
  let agree_prop_par t = match t with
    | Spawn2 (seq_pref,cmds1,cmds2) ->
      ...
      @@ print_triple_vertical ~fig_indent:5 ~res_width:35
        ...
        (pref_obs,obs1,obs2)
    | Spawn3 (seq_pref,cmds1,cmds2,cmds3) ->
      let sut = Spec.init_sut () in
      let pref_obs = interp_sut_res sut seq_pref in
      let wait = Atomic.make 3 in
      let dom1 = Domain.spawn (fun () -> Atomic.decr wait; while 0 <> Atomic.get wait do Domain.cpu_relax() done; try Ok (interp_sut_res sut cmds1) with exn -> Error exn) in
      let dom2 = Domain.spawn (fun () -> Atomic.decr wait; while 0 <> Atomic.get wait do Domain.cpu_relax() done; try Ok (interp_sut_res sut cmds2) with exn -> Error exn) in
      let dom3 = Domain.spawn (fun () -> Atomic.decr wait; while 0 <> Atomic.get wait do Domain.cpu_relax() done; try Ok (interp_sut_res sut cmds3) with exn -> Error exn) in
      let obs1 = Domain.join dom1 in
      let obs2 = Domain.join dom2 in
      ...
      check_quad_obs pref_obs obs1 obs2 obs3 Spec.init_state
      || Test.fail_reportf "  Results incompatible with linearized model\n\n%s"
      @@ print_quad_vertical ~fig_indent:5 ~res_width:35
        (fun (c,r) -> Printf.sprintf "%s : %s" (Spec.show_cmd c) (show_res r))
        (pref_obs,obs1,obs2,obs3)


  let agree_prop_par_asym t = match t with
    | Spawn2 (seq_pref, cmds1, cmds2) ->
      let sut = Spec.init_sut () in
      let pref_obs = interp_sut_res sut seq_pref in
    ...
    | Spawn3 (_seq_pref,_cmds1,_cmds2,_cmds3) ->
      raise (Not_implemented_yet "agree_prop_par_asym Spawn3 case")
